...
export const getWallet = (privateKey?: string) => {
  if (!privateKey) {
    ...
    if (!process.env.PRIVATE_KEY_ADMIN) throw "⛔️ Wallet private key wasn't found in .env file!";
  }

  const provider = getProvider();
  
  // Initialize zkSync Wallet
  const wallet = new Wallet(privateKey ?? process.env.PRIVATE_KEY_ADMIN!, provider);
